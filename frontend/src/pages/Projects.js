...
const Projects = () => {
  ...
  const filteredProjects = projects.filter(project => {
    const matchesSearch = project.input_content.toLowerCase().includes(search.toLowerCase());
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div className="relative flex-1 max-w-md">
            <MagnifyingGlass size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#A3A3A3]" />
            <input
              type="text"
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/flow" className="btn-primary flex items-center gap-2" data-testid="new-project-btn">
              <Plus size={18} />
              Nuevo
            </Link>
          </div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <p className="text-white font-medium mb-2">
                        {project.input_content.substring(0, 100)}
                        {project.input_content.length > 100 ? '...' : ''}
                      </p>
                      <div className="flex flex-wrap items-center gap-3 text-sm">
                        ...
                        <span className={`px-2 py-1 rounded text-xs ${STATUS_BADGES[project.status]?.color || STATUS_BADGES.pending.color}`}>
